...
```text
Старт   — 449 руб. / 60 монет / 30 дней
Базовый — 890 руб. / 150 монет / 30 дней
Про     — 1990 руб. / 360 монет / 30 дней
```
...
| Тариф | Цена | Период | Монет | Эффективная цена 1 монеты |
| --- | ---: | ---: | ---: | ---: |
...
```text
60 сообщений DeepSeek = 60 монет
или 20 сообщений ChatGPT = 60 монет
```

`Базовый` — для регулярного пользователя, который использует текстовые нейросети.

Пример, который укладывается в 150 монет:

```text
150 сообщений DeepSeek = 150 монет
или 50 сообщений ChatGPT = 150 монет
```

`Про` — для активного пользователя, который много работает с текстовыми нейросетями.

Пример, который укладывается в 360 монет:

```text
360 сообщений DeepSeek = 360 монет
или 120 сообщений ChatGPT = 360 монет
```
