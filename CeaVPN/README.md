# CeaAI MVP Documentation

## Быстрый запуск технического MVP

В этом репозитории реализован Telegram-бот CeaAI с SQLite/Postgres, mock-платежами для локальной разработки, YooKassa-платежами для прода, реальными AI-провайдерами DeepSeek/OpenAI для текста, OpenAI GPT Image для картинок, Kling для видео и OpenAI TTS для озвучки.

### Что внутри

- `ceai/bot` — команды, меню и callback handlers Telegram-бота.
- `ceai/services` — бизнес-логика пользователей, подписок, платежей, коинов и генераций.
- `ceai/repositories` — доступ к SQLite-таблицам из модели данных.
- `ceai/providers` — реальные провайдеры DeepSeek/OpenAI для текста, OpenAI GPT Image для картинок, Kling для видео и OpenAI TTS для озвучки.
- `migrations/001_init.sql` — схема БД из `Документация.md`.
- `migrations/postgres/001_init.sql` — та же схема для Postgres.
- `ceai/seed.py` — стартовые тарифы и цены моделей.
- `tests/test_business_logic.py` — минимальная проверка ключевых правил.
- `Dockerfile` — production-образ для Railway/Docker-хостинга.
- `railway.json` — настройки Railway build/deploy и healthcheck.

### Локальный запуск

```bash
python3 -m venv .venv
source .venv/bin/activate
pip install -e .
cp .env.example .env
```

В `.env` нужно указать токен Telegram-бота:

```bash
TELEGRAM_BOT_TOKEN=put-your-telegram-bot-token-here
DATABASE_URL=sqlite:///./data/ceavpn.sqlite3
MOCK_PAYMENT_BASE_URL=https://mock-payments.local/pay
PAYMENT_PROVIDER=mock
AI_PROVIDER_MODE=auto
DEEPSEEK_API_KEY=your-deepseek-api-key
OPENAI_API_KEY=your-openai-api-key
OPENAI_IMAGE_API_KEY=your-openai-image-api-key
KLING_API_KEY=your-kling-api-key
```

## Локальная веб-админка

Админка показывает общую статистику, бесплатных и платных пользователей,
активные подписки, платежи, генерации, баланс и дату регистрации. Из карточки
пользователя можно заблокировать аккаунт или начислить коины; также доступно
переключение технических работ.

По умолчанию панель использует `DATABASE_URL` и доступна только на локальном
компьютере:

```bash
python3 -m ceavpn.admin_web
```

Откройте `http://127.0.0.1:8090`. Чтобы читать боевую PostgreSQL, добавьте в
`.env` её публичный URL отдельно от базы локального бота:

```env
ADMIN_DATABASE_URL=postgresql://user:password@host:5432/dbname
ADMIN_WEB_HOST=127.0.0.1
ADMIN_WEB_PORT=8090
```

Управляющие действия доступны, если в выбранной базе найден активный
администратор из `ADMIN_TELEGRAM_IDS` или `ADMIN_TELEGRAM_USERNAMES`.

Для отдельного веб-сервиса на Railway используйте PostgreSQL и обязательно
задайте два независимых секрета:

```env
ADMIN_WEB_HOST=0.0.0.0
ADMIN_WEB_PORT=8090
ADMIN_WEB_PASSWORD=случайный-пароль-длиной-не-менее-20-символов
ADMIN_WEB_SESSION_SECRET=отдельная-случайная-строка-длиной-не-менее-32-символов
```

Команда сервиса:

```bash
python -m ceavpn.admin_web --host 0.0.0.0 --port 8090
```

При внешнем запуске панель не стартует без PostgreSQL и сильных секретов.
Интерфейс, статические файлы и API закрыты авторизацией; без входа доступен
только безопасный `/healthz` для Railway.

`OPENAI_API_KEY` используется для ChatGPT-текста и OpenAI TTS, `OPENAI_IMAGE_API_KEY` — для GPT Image, `KLING_API_KEY` — для видео через Kling. Если в Railway ключ Kling уже назван иначе, бот также подхватит `KLINGAI_API_KEY`, `KLING_AI_API_KEY`, `KLING_KEY`, `KLING_API`, `KLING_API_TOKEN`, `KLING_TOKEN`, `KLING_ACCESS_KEY`, `KLING_SECRET_KEY` или `API_KEY_KLING`. Если отдельный ключ для картинок не задан, бот использует обычный `OPENAI_API_KEY`. `AI_PROVIDER_MODE=auto` означает: DeepSeek/OpenAI/Kling работают через реальные API при наличии ключей. OpenAI GPT Image, OpenAI TTS и Kling без ключа не отдают mock-результат, а завершают генерацию ошибкой с возвратом коинов. Для полностью тестового режима поставьте `AI_PROVIDER_MODE=mock`.

В разделе озвучки бот отправляет шесть встроенных аудиофайлов одной медиагруппой, а следом — компактное сообщение с кнопками выбора. Файлы хранятся в проекте и никогда не создаются через OpenAI во время работы бота.

Создать БД и заполнить тарифы/модели можно отдельно:

```bash
python -m ceavpn.seed
```

Запуск бота:

```bash
python -m ceavpn.main
```

При старте бот также автоматически применяет миграции и seed-данные.

### Оплата Cea AI через Platega

Для оплаты тарифов основного AI-бота картой и через СБП задайте:

```env
PAYMENT_PROVIDER=platega
PLATEGA_MERCHANT_ID=merchant-id-from-platega
PLATEGA_SECRET=api-key-from-platega
PLATEGA_API_BASE_URL=https://app.platega.io
PLATEGA_WEBHOOK_PATH=/payments/platega/webhook
PLATEGA_RETURN_PATH=/payments/platega/return
PLATEGA_FAILED_PATH=/payments/platega/failed
```

Callback в кабинете Platega:

```text
https://your-service.up.railway.app/payments/platega/webhook
```

Тариф и коины начисляются только после callback, дополнительно проверенного
через API Platega по ID, сумме, валюте и подтверждённому статусу. Повторный
callback не начисляет коины второй раз.

### Деплой на Railway

Railway использует `Dockerfile` и `railway.json`. В проде бот работает через Telegram webhook, поэтому у сервиса должен быть публичный домен.

Один сервис может одновременно обслуживать агрегатор и визуальный VPN-бот. Для
второго бота добавьте в тот же Railway service:

```env
VPN_TELEGRAM_BOT_TOKEN=токен_второго_бота
VPN_BOT_USERNAME=имя_бота_без_собаки
VPN_TELEGRAM_WEBHOOK_SECRET=отдельная_случайная_строка
VPN_SUPPORT_USERNAME=cea_help
VPN_CHANNEL_URL=https://t.me/ceafamily
```

Основной бот продолжит использовать `TELEGRAM_BOT_TOKEN` и путь
`/telegram/webhook`, а VPN-бот получит отдельный webhook
`/telegram/vpn/webhook`. Отдельный Railway service не нужен. Если
`VPN_TELEGRAM_BOT_TOKEN` не задан, запускается только агрегатор.

Оплата VPN через Platega настраивается отдельно и не меняет
`PAYMENT_PROVIDER` основного CeaAI-бота:

```env
VPN_PAYMENT_PROVIDER=platega
VPN_PLATEGA_MERCHANT_ID=merchant-id-from-platega
VPN_PLATEGA_SECRET=secret-from-platega
VPN_PLATEGA_API_BASE_URL=https://app.platega.io
VPN_PLATEGA_WEBHOOK_PATH=/payments/vpn/platega/webhook
VPN_PLATEGA_RETURN_PATH=/payments/vpn/platega/return
VPN_PLATEGA_FAILED_PATH=/payments/vpn/platega/failed
VPN_PLATEGA_REQUEST_TIMEOUT_SECONDS=30
```

В личном кабинете Platega укажите callback URL:

```text
https://your-service.up.railway.app/payments/vpn/platega/webhook
```

Пути `return` и `failed` только возвращают браузер в `@ceavpn_bot`.
Они не выдают VPN и не считаются подтверждением оплаты. Выдача
запускается только после аутентифицированного callback и серверной проверки
статуса, суммы и валюты платежа.

### VPN transport profiles

Первый VPN-узел — один VPS в Амстердаме. В Xray на нём остаются два
транспорта, но Marzban публикует клиенту только один рабочий профиль
`🇳🇱 Нидерланды · Амстердам`:

- `VLESS WS TLS FALLBACK` — публичный WebSocket transport через тот же
  публичный Nginx, который раздаёт подписки:
  `sub.79-137-197-51.sslip.io:8443`;
- `VLESS TCP REALITY` — отключённый в Host Settings технический резерв на
  `79.137.197.51:443`; в подписку Happ он не попадает.

Xray принимает fallback только на `127.0.0.1:10001`; этот порт нельзя открывать
в firewall. Секретный WebSocket path генерируется один раз скриптом
`deploy/vpn/apply-reality-config.sh`, хранится на VPS в root-only файле
`/root/ceavpn-fallback.env` и атомарно подставляется в Xray и активный Nginx
config `/etc/nginx/sites-enabled/ceavpn`. Значение path нельзя коммитить,
печатать в логах или отправлять в чат.

После применения Xray/Nginx config скрипт
`deploy/vpn/configure-marzban-hosts.sh` идемпотентно настраивает Host Settings:
Reality остаётся на `443` с `is_disabled=true`, а публичный TLS WebSocket использует
`8443`. Флаг `🇳🇱` должен быть первым emoji в remark: Happ использует его как
иконку сервера.
Скрипт отправляет в `/api/hosts` только два управляемых inbound tag и проверяет,
что настройки остальных tag не изменились. Worker должен назначать пользователю
оба точных tag через `MARZBAN_INBOUND_TAGS`.

`configure-marzban-hosts.sh` использует отдельную root-only учётную
запись из `/root/ceavpn-sudo-admin.env`, потому что Marzban v0.8.4
разрешает менять Host Settings только sudo-admin. Эти credentials нельзя
копировать в `/root/ceavpn-admin.env` или в environment worker-сервиса.

### Кандидат для режима ограниченного мобильного доступа

`CEAVPN_NODE_MODE=whitelist` — отдельный, по умолчанию не опубликованный
двухузловой режим:

```text
Happ -> VLESS + XHTTP + Reality :443 на проверенном ingress
     -> отдельный VLESS WS/TLS relay :8443 на существующем зарубежном узле
     -> интернет
```

Конфигурация XHTTP следует официальному примеру
[XTLS/Xray-examples](https://github.com/XTLS/Xray-examples/tree/main/VLESS-XHTTP-Reality/minimal-steal_others):
путь XHTTP генерируется один раз, хранится только в
`/root/ceavpn-xhttp.env` с правами `0600`, а Vision flow для этого inbound
отключён. Внешний WS/TLS hop использует отдельный служебный UUID, а не UUID
покупателя.

Для закреплённого Xray server-side Reality target — проверенный удалённый
`COVER_DOMAIN:443` с TLS 1.3/HTTP2 и без redirect. Локальный Nginx target
`127.0.0.1:9443` с XHTTP несовместим. Probe и qualification status живут
отдельно на собственном `SUB_DOMAIN:8443`; сертификат cover на ingress не
выпускается.

Этот режим сам по себе не делает IP доступным у мобильного оператора. HTTPS
probe проверяет только доступность адреса и не доказывает работу VPN. Для
репликации canary-кандидат регистрируется как активный staging worker, но его
нельзя делать canonical server или добавлять в `VPN_EXTRA_PROFILES_JSON`, пока
через
временный XHTTP/Reality canary с нужной SIM при действующих ограничениях не
прошли DNS, Telegram, внешний HTTPS и больше 1 MiB трафика.
В `/root/ceavpn-node.env` такого кандидата обязательно задаётся
`CEAVPN_SERVER_CODE` (например, `ru-wl-1`), в точности совпадающий с
`vpn_servers.code` его worker. Код входит в public profile fingerprint, поэтому
подменить целевой сервер после квалификации нельзя.

Staging worker регистрируется без raw SQL через строгий
`VPN_ADDITIONAL_SERVERS_JSON`; для каждой активной записи обязателен отдельный
secret в `VPN_WORKER_SECRETS_JSON`. Production canonical остаётся
`VPN_SERVER_CODE=nl-1`, а `VPN_EXTRA_PROFILES_JSON=[]` сохраняется до успешного
restricted-SIM canary и статуса `passed`.

Whitelist bundle использует только официальный Xray-core `v26.3.27`.
`deploy/vpn/xray-pins.env` содержит SHA-256 извлечённых Linux executables для
`amd64` и `arm64`. `provision-node.sh` проверяет архитектуру, digest и вывод
`xray version`, затем заменяет executable атомарным `mv`. Старый бинарник
никогда молча не принимается для whitelist-узла. Bundle собирается по
существующему контракту: файлы из `deploy/vpn/` находятся в корне bundle,
Marzban wrapper — в `bundle/marzban`, а содержимое официального release zip —
в `bundle/xray-core/`.

Worker хранит постоянный `VPN_WORKER_EPOCH=e<32 lowercase hex>` в root-only
`/etc/ceavpn/worker.env`. Обычный upgrade сохраняет epoch; явный rebuild
запускает `install-worker.sh ... --rotate-epoch`, после чего Railway заново
реплицирует все активные аккаунты. Whitelist worker без epoch остаётся
fail-closed.

На выбранном существующем зарубежном exit сначала установите
`provision-whitelist-relay.sh` в `/opt/ceavpn/` и создайте отдельный relay:

```bash
sudo /opt/ceavpn/provision-whitelist-relay.sh create \
  --gateway-id ru-candidate-1 \
  --candidate root@candidate.example
```

Скрипт идемпотентно создаёт account только на
`VLESS WS TLS FALLBACK`, не печатает UUID/path и атомарно устанавливает на
candidate `/root/ceavpn-lte-exit.env` с правами `0600`. При ошибке создание или
изменение Marzban откатывается. Для отключения:

```bash
sudo /opt/ceavpn/provision-whitelist-relay.sh revoke \
  --gateway-id ru-candidate-1 \
  --candidate root@candidate.example
```

После provision candidate профиль остаётся `is_disabled=true`. Получите
несекретный probe URL:

```bash
sudo /opt/ceavpn/qualify-whitelist-ingress.sh probe
```

Откройте его с VPN off на реально ограниченной мобильной сети. Это лишь
предварительная проверка. Затем создайте ограниченный canary:

```bash
sudo /opt/ceavpn/qualify-whitelist-ingress.sh canary-create
sudo /opt/ceavpn/qualify-whitelist-ingress.sh canary-status
```

Canary действует 45 минут, ограничен 100 MiB, назначен только на
`VLESS XHTTP REALITY`, а URI записан в root-only
`/root/ceavpn-whitelist-canary.txt` и не печатается. Передайте файл на
доверенное устройство через уже проверенный SSH-канал, импортируйте URI в Happ
и на ограниченной SIM с выключенным Wi-Fi проверьте DNS, Telegram, внешний
HTTPS и передачу больше 1 MiB. Не отправляйте URI в Telegram, чат или облако.
Только после успешного полного tunnel-теста откройте publication gate:

```bash
sudo /opt/ceavpn/qualify-whitelist-ingress.sh pass \
  --operator TELE2 \
  --region Moscow \
  --confirm restricted-sim-xhttp-tunnel-worked
```

`pass` дополнительно требует, чтобы canary был активен недавно и Marzban
зафиксировал строго больше 1 MiB, после чего удаляет canary и его URI. Gate-файл
`/root/ceavpn-whitelist-qualified.json` содержит только статус, время,
оператор, регион, публичный probe URL и названия выполненных проверок — ни XHTTP
path, ни Reality key, ни UUID. `revoke` закрывает gate без удаления
audit-записи. Даже после pass профиль не добавляется в Railway автоматически:
включение сервера и добавление профиля в общую подписку остаются отдельным
production-действием после полного теста приложений, DNS, IPv6 и внешнего IP.

1. Создайте проект Railway из GitHub-репозитория.
2. В сервисе откройте `Settings -> Networking` и нажмите `Generate Domain`.
3. Добавьте Postgres в Railway и подключите `DATABASE_URL` к сервису. В продакшене SQLite запрещён по умолчанию: файл внутри контейнера может быть пересоздан при деплое, и тогда пропадут пользователи, подписки, платежи и балансы.
4. Укажите переменные окружения:

```bash
TELEGRAM_BOT_TOKEN=your-botfather-token
DATABASE_URL=postgresql://user:password@host:5432/dbname
APP_ENV=production
MOCK_PAYMENT_BASE_URL=https://mock-payments.local/pay
PAYMENT_PROVIDER=platega
PLATEGA_MERCHANT_ID=your-platega-merchant-id
PLATEGA_SECRET=your-platega-api-key
TELEGRAM_STARS_AMOUNT=0
AI_PROVIDER_MODE=auto
DEEPSEEK_API_KEY=your-deepseek-api-key
OPENAI_API_KEY=your-openai-api-key
OPENAI_IMAGE_API_KEY=your-openai-image-api-key
KLING_API_KEY=your-kling-api-key
ADMIN_TELEGRAM_USERNAMES=samescam
SUPPORT_USERNAME=cea_help
```

Оставьте `TELEGRAM_STARS_AMOUNT=0`: тогда бот использует отдельную безопасную цену
каждого тарифа в Stars (`319 / 749 / 1599`). Положительное значение больше `1`
переопределяет цену всех тарифов и предназначено только для ручной диагностики.

`APP_BASE_URL` можно не задавать: Railway сам отдаёт `RAILWAY_PUBLIC_DOMAIN`, а приложение превращает его в `https://...` и ставит webhook. Если нужно указать домен вручную:

```bash
APP_BASE_URL=https://your-service.up.railway.app
TELEGRAM_WEBHOOK_PATH=/telegram/webhook
TELEGRAM_WEBHOOK_SECRET=change-me
```

5. После деплоя проверьте:

```text
https://your-service.up.railway.app/healthz
https://your-service.up.railway.app/telegram/status
https://your-service.up.railway.app/payments/yookassa/return
```

Для закрытой диагностики webhook задайте `DIAGNOSTICS_TOKEN` и передавайте его
в заголовке `X-CEA-Diagnostics-Token` при запросе к `/telegram/status`.
Без токена маршруты `/telegram/status` и `/debug/user` намеренно возвращают 404.

### Реальная оплата через YooKassa

Для реальной оплаты используется redirect-сценарий YooKassa:

```text
Бот создает payment в YooKassa
-> пользователь открывает confirmation_url
-> YooKassa присылает webhook payment.succeeded
-> backend проверяет payment через YooKassa API
-> backend начисляет коины один раз
```

Что нужно указать на хостинге:

```bash
PAYMENT_PROVIDER=yookassa
APP_BASE_URL=https://your-service.up.railway.app
YOOKASSA_SHOP_ID=your-shop-id
YOOKASSA_SECRET_KEY=your-secret-key
YOOKASSA_WEBHOOK_PATH=/payments/yookassa/webhook
YOOKASSA_RETURN_PATH=/payments/yookassa/return
```

В личном кабинете YooKassa добавьте входящее уведомление:

```text
Event: payment.succeeded
URL: https://your-service.up.railway.app/payments/yookassa/webhook
```

`/payments/yookassa/return` нужен только как страница возврата пользователя после оплаты. Коины там не начисляются; начисление идет только после webhook и проверки статуса платежа через API YooKassa.

### Оплата криптой через Crypto Pay

Для testnet используйте токен приложения из `@CryptoTestnetBot`.
Для прода используйте `@CryptoBot` и замените API base на `https://pay.crypt.bot/api`.

```bash
CRYPTO_PAY_TOKEN=your-crypto-pay-app-token
CRYPTO_PAY_API_BASE=https://testnet-pay.crypt.bot/api
CRYPTO_PAY_WEBHOOK_SECRET=change-this-long-random-secret
CRYPTO_PAY_WEBHOOK_PATH=/payments/crypto/webhook
CRYPTO_PAY_ACCEPTED_ASSETS=USDT
```

Webhook URL в Crypto Pay:

```text
https://your-service.up.railway.app/payments/crypto/webhook/change-this-long-random-secret
```

Сценарий:

```text
Бот создает invoice в Crypto Pay
-> пользователь открывает pay_url
-> Crypto Pay присылает webhook invoice_paid
-> backend проверяет подпись Crypto-Pay-API-Signature
-> backend начисляет коины один раз
```

### Проверка сценария в Telegram

```text
/start
-> Тарифы
-> выбрать Старт / Базовый / Про
-> Перейти к оплате
-> AI-инструменты
-> выбрать модель
-> отправить prompt
-> получить mock result
-> Баланс / История
```

Чтобы проверить возврат коинов при ошибке AI-провайдера, выберите модель и отправьте prompt `mock_error`.

### Тесты бизнес-логики

```bash
python -m unittest discover -s tests -v
```

Проверяется:

- успешная тестовая оплата начисляет коины один раз;
- повторный webhook не начисляет коины второй раз;
- успешная генерация списывает коины;
- ошибка генерации возвращает коины;
- нельзя генерировать без активной подписки;
- нельзя генерировать при нехватке коинов;
- реферальная ссылка привязывает приглашенного пользователя;
- успешная оплата приглашенного начисляет 30% один раз.

## Суть за 1 минуту

CeaAI - это Telegram-бот, где пользователь покупает подписку, получает коины и тратит их на разные AI-инструменты: чат с нейросетями, генерацию картинок и видео, озвучку текста и работу с контентом.

MVP должен проверить не "можем ли мы подключить много нейросетей", а более важный вопрос: готовы ли пользователи платить за удобный AI-агрегатор прямо в Telegram.

Базовая логика:

```text
Пользователь покупает подписку
    -> получает коины
    -> выбирает AI-инструмент
    -> отправляет запрос
    -> backend проверяет баланс
    -> вызывает нужную нейросеть
    -> бот отправляет результат
```

Главные части MVP:

- Telegram-бот как основной интерфейс;
- backend, где живет вся бизнес-логика;
- база данных для пользователей, подписок, платежей и генераций;
- внешняя платежная система со ссылкой на оплату;
- внутренняя валюта коины;
- адаптеры для подключения разных AI-провайдеров.

## 1. Что такое CeaAI

CeaAI - это Telegram-бот-агрегатор нейросетей. Пользователь покупает подписку, получает внутренние коины/лимиты и тратит их на разные AI-инструменты:

- общение с нейросетями: ChatGPT, Claude, DeepSeek и другие;
- генерация изображений;
- генерация видео;
- озвучка текста;
- обработка и улучшение текста;
- будущие инструменты для маркетинга, SEO, контента и бизнеса.

Главная идея продукта: пользователь не разбирается в разных AI-сервисах, API, VPN, оплатах и подписках. Он открывает одного Telegram-бота, выбирает задачу, отправляет запрос и получает результат.

## 2. Формат продукта

На старте CeaAI делается именно как Telegram-бот, а не как Mini App.

Почему бот:

- быстрее сделать MVP;
- пользователю привычнее писать запросы в чат;
- проще отправлять текст, фото, видео и документы;
- легче тестировать спрос;
- меньше затрат на frontend.

Mini App можно добавить позже как дополнительный интерфейс для:

- красивой галереи генераций;
- истории работ;
- личного кабинета;
- управления подпиской;
- сложных настроек генерации.

## 3. Основной пользовательский сценарий

1. Пользователь открывает Telegram-бота.
2. Нажимает `Start`.
3. Бот показывает приветствие и главное меню.
4. Пользователь видит:
   - баланс коинов;
   - активную подписку;
   - доступные AI-инструменты;
   - кнопку пополнения/покупки подписки.
5. Пользователь покупает подписку через внешнюю платежную ссылку.
6. После успешной оплаты бот начисляет коины.
7. Пользователь выбирает нейросеть или задачу.
8. Бот показывает стоимость действия в коинах.
9. Пользователь отправляет запрос.
10. Backend проверяет подписку и баланс.
11. Backend отправляет запрос в нужный AI-провайдер.
12. Пользователь получает результат в Telegram.
13. С баланса списываются коины.

## 4. Подписки и коины

В CeaAI используется гибридная модель:

- пользователь покупает подписку;
- подписка выдает определенное количество коинов на период;
- коины тратятся на AI-действия;
- если коины закончились, пользователь может докупить пакет коинов или перейти на тариф выше.

Коины нужны, чтобы:

- гибко оценивать разные нейросети;
- не объяснять пользователю себестоимость API;
- контролировать расходы;
- менять цены моделей без изменения логики подписок.

## 5. Тарифы и стоимость действий

Актуальная сетка рассчитана с резервом на реферальное вознаграждение `30%` и
целевую чистую маржу около `40%`. Все тарифы действуют 30 дней.

| Тариф | Цена | Stars | Коинов | Пример максимального использования |
| --- | ---: | ---: | ---: | --- |
| Старт | 299 руб. | 319 | 25 | 25 DeepSeek, 8 ChatGPT/изображений или 1 видео |
| Базовый | 699 руб. | 749 | 60 | 60 DeepSeek, 20 ChatGPT/изображений или 2 видео |
| Про | 1490 руб. | 1599 | 130 | 130 DeepSeek, 43 ChatGPT/изображения или 5 видео |

| Действие | Цена |
| --- | ---: |
| DeepSeek V4 Flash | 1 коин |
| ChatGPT GPT-5.6 | 3 коина |
| GPT Image 2 Medium | 3 коина |
| Kling 3.0, 10 секунд без звука | 25 коинов |
| OpenAI TTS | 3 коина |

Лимиты являются альтернативными максимумами: один набор коинов нельзя одновременно
потратить на все перечисленные количества. GPT Image создаёт `1024x1024`; отдельной
фиктивной цены за «4К» нет.

## 7. Архитектура MVP

Общая схема:

```text
Telegram Bot
    |
    v
CeaAI Backend
    |
    +--> Database
    |
    +--> Payment Provider
    |
    +--> AI Provider Adapters
    |
    +--> File Storage
```

### Telegram Bot

Отвечает за интерфейс:

- главное меню;
- выбор тарифа;
- покупку подписки;
- выбор нейросети;
- прием запросов пользователя;
- отправку результатов;
- уведомления о балансе и ошибках.

Бот не должен напрямую работать с платежками и AI API. Он общается с backend.

### Backend

Главный сервер CeaAI.

Отвечает за:

- пользователей;
- подписки;
- баланс коинов;
- списания и начисления;
- платежные webhook;
- вызовы AI-провайдеров;
- историю генераций;
- обработку ошибок;
- антифрод и лимиты.

### Database

Хранит:

- пользователей;
- подписки;
- тарифы;
- баланс коинов;
- операции с коинами;
- платежи;
- AI-запросы;
- историю генераций;
- настройки моделей.

Для MVP можно использовать SQLite или PostgreSQL. Для продакшена лучше PostgreSQL.

### Payment Provider

Основной сценарий оплаты:

1. Пользователь выбирает тариф.
2. Backend создает платеж.
3. Платежная система возвращает ссылку.
4. Бот отправляет пользователю кнопку `Оплатить`.
5. Пользователь оплачивает на странице платежки.
6. Платежка отправляет webhook на backend.
7. Backend проверяет подпись webhook.
8. Backend активирует подписку и начисляет коины.
9. Бот уведомляет пользователя об успешной оплате.

Возможные платежки:

- ЮKassa;
- CloudPayments;
- Robokassa;
- Т-Банк Касса.

Важно: начислять коины можно только после подтвержденного webhook от платежной системы.

### AI Provider Adapters

Каждая нейросеть подключается через отдельный адаптер.

Пример:

```text
providers/
  openai
  anthropic
  deepseek
  xai
  image
  video
  tts
```

Backend должен иметь единый внутренний интерфейс:

```text
generateText(model, prompt, userContext)
generateImage(model, prompt, options)
generateVideo(model, prompt, options)
generateSpeech(model, text, options)
```

Так бот не зависит от конкретного API. Если одна модель меняется или отключается, остальная система продолжает работать.

## 8. Как подключаются нейросети

Для каждой нейросети нужны:

- аккаунт разработчика;
- API key;
- понимание тарифов API;
- лимиты;
- формат запроса;
- формат ответа;
- обработка ошибок.

Пример логики:

1. Пользователь выбирает `ChatGPT`.
2. Backend смотрит цену модели в коинах.
3. Backend проверяет баланс.
4. Backend резервирует коины.
5. Backend отправляет запрос в OpenAI API.
6. Если ответ успешный, backend списывает коины окончательно.
7. Если произошла ошибка, backend возвращает коины.
8. Бот отправляет ответ пользователю.

Для долгих задач, например видео и сложной озвучки текста, нужна очередь задач.

Пример:

```text
User request
    |
    v
Create generation job
    |
    v
Резерв коинов
    |
    v
Send job to video/tts provider
    |
    v
Wait via polling or webhook
    |
    v
Save result
    |
    v
Send result to Telegram
```

## 9. Что входит в MVP

MVP должен проверить главный вопрос: готовы ли пользователи платить за удобный AI-агрегатор в Telegram.

Минимальный набор:

- Telegram-бот;
- регистрация пользователя после `/start`;
- главное меню;
- баланс коинов;
- 3 тарифа;
- создание платежной ссылки;
- webhook успешной оплаты;
- начисление коинов;
- текстовая нейросеть;
- генерация изображений;
- история последних генераций;
- админская настройка цен хотя бы через конфиг;
- базовые логи ошибок.

В MVP можно не делать:

- сложную Mini App;
- полноценную админ-панель;
- десятки нейросетей;
- многоуровневую реферальную систему;
- командные аккаунты;
- сложную аналитику;
- SEO Pro-инструменты.

## 10. Главное меню бота

Пример структуры:

```text
Главное меню

Баланс: 420 коинов
Подписка: Plus до 15.07.2026

[Нейросети]
[Генерация изображений]
[Видео]
[Музыка]
[Мой баланс]
[Тарифы]
[История]
[Поддержка]
```

## 11. Сценарий покупки подписки

```text
Пользователь -> Тарифы
Бот -> показывает Start / Plus / Pro
Пользователь -> выбирает Plus
Backend -> создает payment pending
Payment Provider -> возвращает payment_url
Бот -> отправляет кнопку оплаты
Пользователь -> оплачивает
Payment Provider -> webhook paid
Backend -> проверяет webhook
Backend -> активирует подписку
Backend -> начисляет коины
Бот -> "Оплата прошла. Вам начислено 60 коинов."
```

## 12. Сценарий генерации текста

```text
Пользователь -> Нейросети
Пользователь -> ChatGPT
Бот -> "Отправьте запрос. Стоимость: 3 коина."
Пользователь -> пишет запрос
Backend -> проверяет подписку
Backend -> проверяет баланс
Backend -> отправляет запрос в AI API
AI Provider -> возвращает ответ
Backend -> списывает коины
Backend -> сохраняет историю
Бот -> отправляет ответ
```

## 13. Сценарий генерации изображения

```text
Пользователь -> Генерация изображений
Бот -> "Опишите картинку. Стоимость: 5 коинов."
Пользователь -> пишет промпт
Backend -> проверяет баланс
Backend -> резервирует коины
Backend -> отправляет запрос в image API
AI Provider -> возвращает image_url или файл
Backend -> сохраняет результат
Backend -> списывает коины
Бот -> отправляет изображение
```

## 14. Основные таблицы базы данных

### users

Пользователи Telegram.

Поля:

- id;
- telegram_id;
- username;
- first_name;
- created_at;
- last_seen_at;
- status.

### plans

Тарифы.

Поля:

- id;
- code;
- name;
- price_rub;
- duration_days;
- coins_amount;
- is_active.

### subscriptions

Активные подписки пользователей.

Поля:

- id;
- user_id;
- plan_id;
- starts_at;
- ends_at;
- status.

### coin_transactions

Все движения коинов.

Поля:

- id;
- user_id;
- amount;
- type;
- reason;
- related_entity_id;
- created_at.

Типы:

- payment_bonus;
- generation_charge;
- refund;
- manual_adjustment;
- subscription_grant.

### payments

Платежи.

Поля:

- id;
- user_id;
- provider;
- provider_payment_id;
- amount_rub;
- status;
- payment_url;
- plan_id;
- created_at;
- paid_at.

### ai_models

Настройки моделей.

Поля:

- id;
- provider;
- model_code;
- display_name;
- type;
- coin_price;
- is_active.

Типы:

- text;
- image;
- video;
- tts.

### generations

История генераций.

Поля:

- id;
- user_id;
- model_id;
- type;
- prompt;
- result_url;
- result_text;
- status;
- coins_spent;
- error_message;
- created_at;
- completed_at.

## 15. Правила списания коинов

Для быстрых задач:

1. Проверить баланс.
2. Выполнить запрос.
3. Списать коины после успешного результата.

Для дорогих или долгих задач:

1. Проверить баланс.
2. Зарезервировать коины.
3. Запустить генерацию.
4. Если успешно - списать резерв.
5. Если ошибка - вернуть резерв.

Это важно, чтобы пользователь не терял коины из-за ошибок провайдера.

## 16. Обработка ошибок

Примеры ошибок:

- недостаточно коинов;
- подписка закончилась;
- AI-провайдер недоступен;
- платеж не подтвержден;
- пользователь отправил неподдерживаемый файл;
- генерация заняла слишком много времени.

Бот должен отвечать простым языком.

Пример:

`Не получилось сгенерировать изображение. Коины не списаны. Попробуйте еще раз чуть позже.`

## 17. Админ-функции

На старте можно сделать админ-функции через команды в Telegram или через конфиг.

Нужно уметь:

- смотреть пользователя;
- менять баланс вручную;
- включать/выключать модель;
- менять стоимость модели;
- смотреть последние платежи;
- смотреть ошибки генераций;
- выдавать промокод или бонус.

Позже можно сделать отдельную веб-админку.

## 18. Метрики MVP

Важно отслеживать:

- сколько пользователей нажали `/start`;
- сколько пользователей попробовали бесплатные коины;
- сколько дошли до оплаты;
- конверсия в оплату;
- средний чек;
- какие модели используют чаще;
- сколько стоит один активный пользователь;
- сколько API-расходов на пользователя;
- маржа по тарифам;
- сколько пользователей возвращаются на следующий день.

## 19. Главные риски

### Экономика

Если неправильно посчитать стоимость API, можно продавать подписки дешевле себестоимости.

Решение:

- считать примерную себестоимость каждого действия;
- ставить запас по марже;
- быстро менять цены в коинах.

### Стабильность AI API

Некоторые провайдеры могут тормозить, падать или менять цены.

Решение:

- делать адаптеры;
- логировать ошибки;
- возвращать коины при сбоях;
- иметь альтернативные модели.

### Сложность продукта

Если сразу добавить много нейросетей, продукт станет сложным и неудобным.

Решение:

- начать с небольшого набора популярных сценариев;
- сделать понятное меню;
- добавлять модели по спросу.

## 20. Дорожная карта

### Этап 1: Документация и проектирование

- описать механику продукта;
- определить тарифы;
- определить первые модели;
- выбрать платежку;
- выбрать технический стек.

### Этап 2: MVP бота

- Telegram-бот;
- база пользователей;
- баланс коинов;
- тарифы;
- платежная ссылка;
- webhook оплаты;
- одна текстовая модель;
- одна модель генерации изображений.

### Этап 3: Улучшение продукта

- история генераций;
- расширение реферальной системы;
- промокоды;
- больше моделей;
- очередь для видео и долгих задач озвучки текста;
- админ-функции.

### Этап 4: Масштабирование

- Mini App;
- веб-кабинет;
- SEO Pro-инструменты;
- командные аккаунты;
- аналитика;
- партнерская программа.

## 21. Принцип разработки

CeaAI нужно строить не как набор случайно подключенных API, а как платформу.

Правильная логика:

```text
Telegram - это интерфейс.
Backend - это мозг продукта.
AI-провайдеры - это заменяемые модули.
Коины - это внутренняя экономика.
Подписка - это основная модель монетизации.
```

Такой подход позволит сначала быстро запустить Telegram-бота, а потом без полной переделки добавить Mini App, сайт, админку и новые нейросети.
