...
from ceaai.config import DEFAULT_INFO_CHANNEL_URL, DEFAULT_PUBLIC_OFFER_URL, Settings
from ceaai.database import Database
from ceaai.internal_api import (
    handle_provider_settings_request,
    handle_provider_status_request,
)
from ceaai.json_utils import dumps, loads_dict
from ceaai.repositories.app_settings import AppSettingsRepository
from ceaai.providers.base import ImageInput, ProviderError, ProviderResult
from ceaai.providers.deepseek import DeepSeekProvider
from ceaai.providers.kling_video import KlingVideoProvider
from ceaai.providers.openai_image import OpenAIImageProvider
from ceaai.providers.openai_text import OpenAITextProvider
from ceaai.providers.openai_tts import OpenAITTSProvider
from ceaai.providers.router import AIProviderRouter
from ceaai.seed import seed_reference_data
from ceaai.services.app import build_services
from ceaai.services.exceptions import (
    BusinessRuleError,
    GenerationProviderFailedError,
    InsufficientCoinsError,
    NoActiveSubscriptionError,
)
from ceaai.time_utils import iso_now, utcnow
...
class BusinessLogicTest(unittest.TestCase):
    ...
    def test_yookassa_network_errors_are_retried_and_sanitized(self) -> None:
        ...
        with patch(
            "ceaai.services.payments.urllib.request.urlopen",
            side_effect=error,
        ) as urlopen, patch("ceaai.services.payments.time.sleep") as sleep:
    ...
    def test_successful_yookassa_webhook_credits_coins_once(self) -> None:
        from ceaai.payment_notifications import format_payment_notification
    ...
    def test_profile_counts_invited_users_and_links_account_name(self) -> None:
        from ceaai.bot.handlers import (
            _format_menu,
    ...
    def test_image_provider_errors_are_user_readable(self) -> None:
        from ceaai.services.generations import _provider_error_message
    ...
    def test_handlers_show_provider_failure_text(self) -> None:
        handlers_source = Path("ceaai/bot/handlers.py").read_text(encoding="utf-8")
...
class MigrationAndUITest(unittest.TestCase):
    def test_reply_keyboard_has_no_audio_button_but_has_tts_button(self) -> None:
        keyboard_source = Path("ceaai/bot/keyboards.py").read_text(encoding="utf-8")
    ...
    def test_ai_reply_keyboards_have_back_button_only(self) -> None:
        from ceaai.bot.handlers import _format_direct_prompt_screen
        from ceaai.bot.handlers import _format_media_generation_caption
        from ceaai.bot.keyboards import model_choice_label

        keyboard_source = Path("ceaai/bot/keyboards.py").read_text(encoding="utf-8")
        handlers_source = Path("ceaai/bot/handlers.py").read_text(encoding="utf-8")
        seed_source = Path("ceaai/seed.py").read_text(encoding="utf-8")
    ...
    def test_video_and_tts_sections_are_enabled(self) -> None:
        ...
        for voice in ("alloy", "echo", "fable", "onyx", "nova", "shimmer"):
            self.assertTrue(Path(f"ceaai/assets/tts_voices/{voice}.wav").is_file())
        self.assertIn("reply_markup=back_to_menu_keyboard()", handlers_source)
    ...
    def test_plan_screen_uses_new_prices_and_coins_are_called_koiny(self) -> None:
        ...
        from ceaai.bot.keyboards import (
        ...
        self.assertIn("После оплаты начисляется 25 коинов", start_details)
        self.assertIn(
            "Куда можно потратить (если выбрать один инструмент):",
            start_details,
        )
        self.assertIn("до 25 запросов DeepSeek", start_details)
        self.assertIn("до 8 запросов ChatGPT, изображений или озвучек", start_details)
        self.assertIn("до 1 видео Kling", start_details)
        self.assertNotIn("<b>", start_details)
        self.assertNotIn("➕", start_details)
        self.assertIn("💳 Выберите способ оплаты:", start_details)
        self.assertIn("Тариф: Базовый", basic_details)
        self.assertIn("Стоимость: 699 ₽ или 749 ⭐", basic_details)
        self.assertIn("После оплаты начисляется 60 коинов", basic_details)
        self.assertIn("до 60 запросов DeepSeek", basic_details)
        self.assertIn("до 20 запросов ChatGPT, изображений или озвучек", basic_details)
        self.assertIn("до 2 видео Kling", basic_details)
        self.assertNotIn("<b>", basic_details)
        self.assertNotIn("➕", basic_details)
        self.assertIn("Тариф: Про", pro_details)
        self.assertIn("Стоимость: 1490 ₽ или 1599 ⭐", pro_details)
        self.assertIn("После оплаты начисляется 130 коинов", pro_details)
        self.assertIn("до 130 запросов DeepSeek", pro_details)
        self.assertIn("до 43 запросов ChatGPT, изображений или озвучек", pro_details)
        self.assertIn("до 5 видео Kling", pro_details)
        self.assertNotIn("<b>", pro_details)
    ...
    def test_telegram_commands_menu_contains_only_start_and_profile(self) -> None:
        main_source = Path("ceaai/main.py").read_text(encoding="utf-8")
        ...
        self.assertIn("🔥 Cea AI — современные AI-инструменты в одном боте.", main_source)
        self.assertIn("🚀 Нажмите /start, чтобы начать.", main_source)
    ...
    def test_admin_inline_buttons_edit_existing_screen(self) -> None:
        handlers_source = Path("ceaai/bot/handlers.py").read_text(encoding="utf-8")
        keyboards_source = Path("ceaai/bot/keyboards.py").read_text(encoding="utf-8")
    ...
    def test_menu_command_has_main_menu_copy(self) -> None:
        ...
        self.assertIn("GIFT_DURATION_DAYS = 30", handlers_source)
        ...
        self.assertIn("Бесплатный доступ активирован", handlers_source)
        self.assertNotIn("Пробный доступ активирован на", handlers_source)
        self.assertNotIn("После подписки нажмите проверку", handlers_source)
    ...
    def test_profile_screen_has_inline_actions_and_no_bottom_prompt(self) -> None:
        from ceaai.bot.keyboards import profile_keyboard, referral_keyboard
    ...
    def test_media_result_links_are_hidden_behind_short_labels(self) -> None:
        from ceaai.bot.handlers import _format_generation_result
        from ceaai.bot.handlers import _format_history_result
        from ceaai.bot.handlers import _format_video_generation_result
    ...
    def test_history_screen_has_paged_generation_buttons(self) -> None:
        from ceaai.bot.handlers import _format_history, _format_history_result
        from ceaai.bot.keyboards import history_keyboard, history_result_keyboard
    ...
    def test_subscription_required_message_has_plan_buttons_without_test_copy(self) -> None:
        from ceaai.bot.handlers import _subscription_required_message
        from ceaai.bot.keyboards import (
    ...
    def test_onboarding_env_settings_are_read(self) -> None:
        from ceaai.config import load_settings
        ...
        with (
            patch("ceaai.config._load_dotenv", return_value={}),
    ...
    def test_production_refuses_ephemeral_sqlite(self) -> None:
        from ceaai.main import _ensure_persistent_database
    ...
    def test_text_provider_instructions_identify_chatgpt_and_deepseek(self) -> None:
        from ceaai.providers.identity import text_model_instructions
