...
from ceaai.config import DEFAULT_PRIVACY_POLICY_URL, DEFAULT_PUBLIC_OFFER_URL
from ceaai.formatting import (
    format_coin_amount,
    format_datetime_minute,
    format_datetime_russian_minute,
)
from ceaai.json_utils import loads_dict
from ceaai.pricing import telegram_stars_amount_for_rub
from ceaai.providers.base import ImageInput
from ceaai.runtime_diagnostics import record_error, record_message
from ceaai.services.app import AppServices
...
from ceaai.services.referrals import (
...
async def _handle_reply_menu(
    ...
    if text_lower in {"заработать", "реферальная программа"} or text == REFERRAL_BUTTON:
        _clear_dialog_state(services, user["id"])
        await message.answer("🛠 Раздел «Заработать» временно недоступен. Ведутся технические работы.")
        return True
