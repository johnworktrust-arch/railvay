...
```text
Старт   — 299 руб. / 25 коинов / 30 дней / 319 Stars
Базовый — 699 руб. / 60 коинов / 30 дней / 749 Stars
Про     — 1490 руб. / 130 коинов / 30 дней / 1599 Stars
```
...
Цель расчёта: около 40% чистой маржи после резерва 30% на реферальное
вознаграждение. Для валютных API используется плановый курс 85 руб./USD.

### Цены AI-действий

| Тип | Провайдер / модель | Расчётная себестоимость | Коинов |
| --- | --- | ---: | ---: |
| Текст | DeepSeek V4 Flash | зависит от токенов, обычно менее 0.1 руб. | 1 |
| Текст | ChatGPT GPT-5.5 | зависит от токенов, установлен лимит ответа | 3 |
| Картинка | GPT Image 2 Medium, 1024x1024 | около $0.053 | 3 |
| Видео 10 сек | Kling 3.0 Standard без звука | 6 units × $0.098 = $0.588 | 25 |
| Озвучка | OpenAI TTS | $15 за 1 млн символов | 3 |

Отдельного режима 4К нет: GPT Image 2 сейчас всегда вызывается в размере
`1024x1024`, поэтому упоминание «4К» в prompt не меняет цену.

### Тарифы

| Тариф | Цена | Telegram Stars | Период | Коинов | Максимальный пример |
| --- | ---: | ---: | ---: | ---: | --- |
| Старт | 299 руб. | 319 Stars | 30 дней | 25 | 25 DeepSeek, 8 ChatGPT/изображений или 1 видео |
| Базовый | 699 руб. | 749 Stars | 30 дней | 60 | 60 DeepSeek, 20 ChatGPT/изображений или 2 видео |
| Про | 1490 руб. | 1599 Stars | 30 дней | 130 | 130 DeepSeek, 43 ChatGPT/изображения или 5 видео |

Перечисленные количества являются альтернативными максимумами. Например, 60
коинов нельзя одновременно потратить на 20 изображений и 2 видео.

### Учёт фактической себестоимости

В `generations` сохраняются `provider_cost_amount` и
`provider_cost_currency`. В `result` дополнительно сохраняется usage:

- токены ChatGPT и DeepSeek;
- входные и выходные токены GPT Image;
- количество символов OpenAI TTS;
- ресурсные units Kling.

Это позволяет пересчитывать реальную маржу по каждой генерации. Для защиты от
аномально дорогих запросов текст ChatGPT/DeepSeek ограничен 6000 символами,
ответ ChatGPT — 1500 токенами, DeepSeek — 2000 токенами.
